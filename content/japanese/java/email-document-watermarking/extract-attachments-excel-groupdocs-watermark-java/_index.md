...
## クイック回答
- **Excel添付ファイルを扱うライブラリはどれですか？** GroupDocs.Watermark for Java。
- **スプレッドシートを読み込むメソッドはどれですか？** `new Watermarker(filePath, new SpreadsheetLoadOptions())`。
- **Javaでワークシートを反復処理できますか？** はい。`content.getWorksheets()`を使用して、各`SpreadsheetWorksheet`をループ処理します。
- **Is a license required for production?** 本番使用にはフルの GroupDocs.Watermark ライセンスが必要です。  
- **大きなファイルでも動作しますか？** はい、Watermarkerを速やかに閉じ、適切な読み込みオプションを使用すれば動作します。
...
### Maven 設定
`pom.xml`にGroupDocsリポジトリと依存関係を追加してください。
...
### 基本的な初期化と設定
Excelファイルを指す`Watermarker`インスタンスを作成します。
...
### スプレッドシートのロードと準備
まず、`SpreadsheetLoadOptions` を使用してワークブックを読み込み、ライブラリが Excel ファイルであることを認識できるようにします。
...
### スプレッドシートコンテンツへのアクセス
ワークシートとその添付ファイルにアクセスできる上位レベルのコンテンツオブジェクトを取得します。
...
各ワークシートをループ処理し、次にそのシート内の各添付ファイルをループ処理します。
...
### 添付ファイルの詳細抽出
各`SpreadsheetAttachment`について、メタデータ、プレビュー画像、および生のファイルバイトを読み取ることができます。
...
### リソースのクローズ
メモリを解放するために、作業が完了したら必ず「ウォーターマーカー」を解放してください。
