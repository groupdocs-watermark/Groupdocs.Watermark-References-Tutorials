...
1. **我可以将 GroupDocs.Watermark 用于其他文件格式吗？**  
   是的，它支持除图表之外的多种文档类型，包括 PDF、DOCX、PPTX 等。  
2. **我可以添加的水印数量有限制吗？**  
   没有硬性限制，但添加大量复杂水印可能会影响性能。  
3. **如何从图表中移除水印？**  
   该库提供检测和移除的 API；详情请参阅官方文档。  
4. **文字水印可以添加到所有页面还是仅选定页面？**  
   您可以通过配置 `DiagramShapeWatermarkOptions` 来针对特定页面或形状。  
5. **如果水印未按预期显示，我该怎么办？**  
   检查放置设置、字体颜色对比度，并确保在添加水印后已保存文件。  
