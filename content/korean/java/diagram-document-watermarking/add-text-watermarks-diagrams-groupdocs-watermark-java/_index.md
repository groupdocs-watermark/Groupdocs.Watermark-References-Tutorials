...
## FAQ 섹션
1. **GroupDocs.Watermark를 다른 파일 형식에도 사용할 수 있나요?**  
   네, 다이어그램 외에도 PDF, DOCX, PPTX 등 다양한 문서 유형을 지원합니다.  
2. **추가할 수 있는 워터마크 수에 제한이 있나요?**  
   명확한 제한은 없지만, 복잡한 워터마크를 많이 추가하면 성능에 영향을 줄 수 있습니다.  
3. **다이어그램에서 워터마크를 제거하려면 어떻게 하나요?**  
   라이브러리는 탐지 및 제거 API를 제공하므로 공식 문서를 참고하세요.  
4. **텍스트 워터마크를 모든 페이지에 추가할 수 있나요, 아니면 선택된 페이지에만 추가할 수 있나요?**  
   `DiagramShapeWatermarkOptions`를 설정하여 특정 페이지나 도형을 대상으로 할 수 있습니다.  
5. **워터마크가 예상대로 표시되지 않으면 어떻게 해야 하나요?**  
   배치 설정, 폰트 색상 대비를 확인하고 워터마크를 추가한 후 파일을 저장했는지 확인하세요.
